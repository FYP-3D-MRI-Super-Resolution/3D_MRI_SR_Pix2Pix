...
for l in ${lone[@]}; do
  for i in ${!ltwo[@]}; do
    python3 test.py --dataset_mode nifti --model pix2pix3d --name t1t2_unet --gpu_id ${i} --fp16 \
    ...
    --dataroot /input --checkpoints_dir /checkpoints --results_dir /results > "/results/unet_out_${l}_${ltwo[$i]}.txt" & 
  done
  wait
done
